...
export async function generateMetadata({
  ...
  if (params?.category) {
    return {
      title: `${categories.find((category) => category.slug == params?.category)?.title as string} - THE OFFSHOP`,
    };
...
export default function Category({
  ...
  const products = params?.category
    ? catalog.filter(({ categories }) => categories.includes(params?.category))
    : catalog;
  ...
  return (
    <section>
      <h1 className="text-2xl font-semibold text-center mb-8">
        {categories.find((category) => category.slug == params?.category)?.title || 'Catálogo'}
      </h1>
      {searchParams?.search && (
      ...
      {totalPages > 1 && (
        <div className="join flex justify-center">
          {page > 1 ? (
            <Link
              href={`?${params?.category ? `category=${params.category}&` : ''}${searchParams?.search ? `search=${searchParams.search}&` : ''}page=${prevPage}`}
          ...
          {page < totalPages ? (
            <Link
              href={`?${params?.category ? `category=${params.category}&` : ''}${searchParams?.search ? `search=${searchParams.search}&` : ''}page=${nextPage}`}
              className="join-item btn btn-ghost"
            >
              »
            </Link>
